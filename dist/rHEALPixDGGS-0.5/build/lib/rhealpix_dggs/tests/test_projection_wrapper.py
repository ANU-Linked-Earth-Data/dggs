...
import ellipsoids as ell
from pj_rhealpix import rhealpix_ellipsoid, rhealpix_ellipsoid_inverse
...
#------------------------------------------------------------------------------# Test Proj class.
    def test_call(self):
        ...
        f = pw.Proj(ellipsoid=E, proj='rhealpix', 
        ...
        for p in sample:
            # Forward.
            get = f(*p)
            expect = rhealpix_ellipsoid(*p, north_square=ns, south_square=ss, 
                                        e=e)
            ...
            expect = rhealpix_ellipsoid_inverse(*expect, 
                                                north_square=ns, 
                                                south_square=ss, e=e)
            expect = tuple(expect)
            self.assertEqual(get, expect)
        ...
        f_deg = pw.Proj(ellipsoid=E_DEG, proj='rhealpix', 
                     north_square=ns, south_square=ss)
        ...
        f = pw.Proj(ellipsoid=E_DEG, proj='rhealpix', 
                 north_square=ns, south_square=ss)
        self.assertEqual(f(*p), z)
        self.assertEqual(f(*z, inverse=True), p)
#------------------------------------------------------------------------------                           
