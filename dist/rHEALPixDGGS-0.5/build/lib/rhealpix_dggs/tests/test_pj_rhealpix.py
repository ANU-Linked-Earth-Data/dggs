...
class MyTestCase(unittest.TestCase):
    def test_triangle(self):
        ...
        for (north_square, south_square) in product(list(range(4)), repeat=2): 
            for p in eq_points:
                cr = pjr.triangle(*p, north_square=north_square, 
    ...
    def test_combine_triangles(self):
        ...
        for (north_square, south_square) in product(list(range(4)), repeat=2): 
            for p in eq_points:
                q = pjr.combine_triangles(*p, north_square=north_square, 
                                          south_square=south_square)
        ...
        for (north_square, south_square) in product(list(range(4)), repeat=2): 
            for p in eq_points + sp_points + np_points:
                q = pjr.combine_triangles(*p, south_square=south_square, 
                                 north_square=north_square)
                pp = pjr.combine_triangles(*q, south_square=south_square, 
                                  north_square=north_square, inverse=True)
        ...
        for (north_square, south_square) in product(list(range(4)), repeat=2): 
            ...
            for q in np_boundary:
                p = pjr.combine_triangles(*q, south_square=south_square, 
                                 north_square=north_square, inverse=True)
