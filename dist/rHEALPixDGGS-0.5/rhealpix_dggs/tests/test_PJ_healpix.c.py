...
class PJHEALPixTestCase(unittest.TestCase):
    ...
    def test_rhealpix_sphere(self):
        ...
        for (ns, ss) in product(list(range(4)), repeat=2): 
            ...
            rh = Proj(proj='rhealpix', R=R, north_square=ns, 
                      south_square=ss)
        ...
        dx = pi/4
        dy = pi/12
        north_healpix_output = [R*array((-pi + i*pi/2 + dx, pi/4 + dy)) 
                                for i in range(4)]
        south_healpix_output = [R*array((-pi + i*pi/2 + dx, -pi/4 - dy)) 
                                for i in range(4)] 
        h = Proj(proj='healpix', R=R)
        north_given = [h(*p, radians=RADIANS, inverse=True) 
                       for p in north_healpix_output]
        south_given = [h(*p, radians=RADIANS, inverse=True) 
                       for p in south_healpix_output]        
        for (ns, ss) in product(list(range(4)), repeat=2): 
            print('_____ north_square = %s, south_square = %s' % (ns, ss))
            rh = Proj(proj='rhealpix', R=R, north_square=ns, 
                      south_square=ss)          
        ...
        for (ns, ss) in product(list(range(4)), repeat=2): 
            print('_____ north_square = %s, south_square = %s' % (ns, ss))
            f = Proj(proj='rhealpix', R=R, north_square=ns, 
                     south_square=ss)
            for p in inputs:
                expect = p
