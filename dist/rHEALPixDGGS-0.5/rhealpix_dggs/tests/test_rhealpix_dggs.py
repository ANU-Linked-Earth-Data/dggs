...
# Import my modules. 
import rhealpix_dggs as rd
from rhealpix_dggs import RHEALPixDGGS, Cell, WGS84_003, WGS84_003_RADIANS
from ellipsoids import Ellipsoid, WGS84_ASPHERE_RADIANS, WGS84_ELLIPSOID, WGS84_ELLIPSOID_RADIANS 
...
WGS84_123 = rd.RHEALPixDGGS(ellipsoid=WGS84_ELLIPSOID, 
                         north_square=1, south_square=2, N_side=3)
WGS84_123_RADIANS = rd.RHEALPixDGGS(ellipsoid=WGS84_ELLIPSOID_RADIANS, 
                                 north_square=1, south_square=2, N_side=3)
...
#------------------------------------------------------------------------------            
    ...
    def test_suid_rowcol(self):
        for rdggs in [WGS84_123, WGS84_123_RADIANS]:
            ...
            for suid in CELLS0:
                self.assertEqual(suid, Cell(rdggs, 
                                 (suid,)).suid_rowcol()[0][0])
                self.assertEqual(suid, Cell(rdggs,      
                                            (suid,)).suid_rowcol()[1][0])
    ...
    def test_cell_from_region(self):
        for rdggs in [WGS84_003, WGS84_003_RADIANS]:
            ...
            expect = [O, 3, 0, 1] +\
                     [4 for i in range(rdggs.max_resolution - 3)]
    ...
    def test_cell_latitudes(self):
        for rdggs in [WGS84_003, WGS84_003_RADIANS]:
            ...
            R_A = rdggs.ellipsoid.R_A
            resolution = 1
            get = rdggs.cell_latitudes(resolution, -R_A*pi/2, R_A*pi/2, True, 
                                       plane=True)
            expect = R_A*array([-pi/3, -pi/6, 0, pi/6, pi/3,])
            for i in range(len(expect)):
                self.assertAlmostEqual(get[i], expect[i])

            get = rdggs.cell_latitudes(resolution, -R_A*pi/3, R_A*pi/3, True, 
                                       plane=True)
            ...
            resolution = 1
            get = rdggs.cell_latitudes(resolution, -R_A*pi/2, R_A*pi/2, False, 
                                       plane=True)
            expect = R_A*array([-5*pi/12, -pi/4, -pi/12, pi/12, pi/4, 5*pi/12])
            for i in range(len(get)):
                self.assertAlmostEqual(get[i], expect[i])
    
            get = rdggs.cell_latitudes(resolution, -R_A*pi/4, R_A*5*pi/12, False,
                                       plane=True)
    ...
    def test_cells_from_meridian(self):
        for rdggs in [WGS84_003, WGS84_003_RADIANS]:
            ...
            expect = [
                rdggs.cell([N, 4]), rdggs.cell([N, 8]), rdggs.cell([N, 5]),
                rdggs.cell([P, 0]), rdggs.cell([P, 3]), rdggs.cell([P, 6]),
                rdggs.cell([S, 2]), rdggs.cell([S, 5]), rdggs.cell([S, 4])
            ] 
            self.assertEqual(get, expect)
    ...
    def test_cells_from_parallel(self):
        for rdggs in [WGS84_003, WGS84_003_RADIANS]:
            ...
            expect = [
                rdggs.cell([N, 6]), rdggs.cell([N, 7]), rdggs.cell([N, 8]),
                rdggs.cell([N, 5]), rdggs.cell([N, 2]), rdggs.cell([N, 1]),
                rdggs.cell([N, 0]), rdggs.cell([N, 3])
            ] 
            self.assertEqual(get, expect)
    ...
    def test_cells_from_region(self):
        for rdggs in [WGS84_003, WGS84_003_RADIANS]:
            ...
            expect = [
              ...
              [rdggs.cell((Q, 0)), rdggs.cell((Q, 1)), rdggs.cell((Q, 2)), 
               rdggs.cell((R, 0))],
            ]
            ...
            get = rdggs.cells_from_region(1, ul, dr, plane=False)
            expect = [
              [rdggs.cell((Q, 6)), rdggs.cell((Q, 7)), rdggs.cell((Q, 8)),
               rdggs.cell((R, 6))],
            ...
            expect = [
              ...
              [rdggs.cell((N, 6)), rdggs.cell((N, 7)), rdggs.cell((N, 8)), 
               rdggs.cell((N, 5)), rdggs.cell((N, 2)), rdggs.cell((N, 1)),
               rdggs.cell((N, 0)), rdggs.cell((N, 3))],
              [rdggs.cell((O, 0)), rdggs.cell((O, 1)), rdggs.cell((O, 2)),
               rdggs.cell((P, 0)), rdggs.cell((P, 1)), rdggs.cell((P, 2)),
               rdggs.cell((Q, 0)), rdggs.cell((Q, 1)), rdggs.cell((Q, 2)),
               rdggs.cell((R, 0)), rdggs.cell((R, 1)), rdggs.cell((R, 2))],
            ]
            self.assertEqual(get, expect)
            ...
            get = rdggs.cells_from_region(1, ul, dr, plane=False)
            expect = [
               [rdggs.cell((S, 0)), rdggs.cell((S, 1)), rdggs.cell((S, 2)), 
                rdggs.cell((S, 5)), rdggs.cell((S, 8)), rdggs.cell((S, 7)),
                rdggs.cell((S, 6)), rdggs.cell((S, 3))],
               [rdggs.cell((S, 4))],
            ]
            self.assertEqual(get, expect)
